...
class LatentDiffusion(DDPM):
    ...
    def training_step(self, batch, batch_idx): #training_step -> LatentVisualDiffusion.shared_step ->LatentDiffusion.forward -> p_losses -> get_loss -> mse_loss
...
class LatentVisualDiffusion(LatentDiffusion):
    ...
    def shared_step(self, batch, random_uncond, **kwargs):
        x, c, fs = self.get_batch_input(batch, random_uncond=random_uncond, return_fs=True, action_random_drop_btw_timestep=self.action_random_drop_btw_timestep)
    ...
    def log_images(self, batch, sample=True, ddim_steps=50, ddim_eta=1., plot_denoise_rows=False, \
                    unconditional_guidance_scale=1.0, mask=None, sampled_img_num=None, ar=False, ar_noise_schedule=None, cond_frame=None, **kwargs):
        """ log images for LatentVisualDiffusion """
        ##### sampled_img_num: control sampled imgae for logging, larger value may cause OOM
        if sampled_img_num is None:
            sampled_img_num = 1
        else:
            sampled_img_num = sampled_img_num
        for key in batch.keys():
            batch[key] = batch[key][:sampled_img_num]
        ...
        if sample:
            ...
            if unconditional_guidance_scale != 1.0:
                ...
                ## hybrid case
                if isinstance(c, dict):
                    if 'c_action' in c.keys():
                        if 'c_action_mask' in c.keys():
                            c_action_mask = c['c_action_mask'][0]
                            
                            random_mask = (torch.rand_like(c_action_mask, device=c_action_mask.device) > 0.2).to(c_action_mask.device)
                            uc_hybrid = {"c_concat": [c_cat], "c_crossattn": [uc], "c_action": c['c_action'], "c_action_mask": [random_mask]}
                            # uc_hybrid = {"c_concat": [c_cat], "c_crossattn": [uc], "c_action": c['c_action'], "c_action_mask": [c_action_mask]}
                        else:
                            uc_hybrid = {"c_concat": [c_cat], "c_crossattn": [uc], "c_action": c['c_action']}
                    else:
                        uc_hybrid = {"c_concat": [c_cat], "c_crossattn": [uc]}
                    uc = uc_hybrid
            else:
            ...
            with self.ema_scope("Plotting"):
                samples, z_denoise_row = self.sample_log(cond=c, batch_size=N, ddim=use_ddim,
                                                         ddim_steps=ddim_steps,eta=ddim_eta,
                                                         unconditional_guidance_scale=unconditional_guidance_scale,
                                                         unconditional_conditioning=uc, x0=z, ar=ar, ar_noise_schedule=ar_noise_schedule, cond_frame=cond_frame, z=z, **kwargs)
            x_samples = self.decode_first_stage(samples)
            log["samples"] = x_samples
