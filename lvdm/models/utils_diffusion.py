...
def timestep_embedding(timesteps, dim, max_period=10000, repeat_only=False):
    ...
    :param timesteps: a Tensor of shape [B] or [B, T] containing indices.
                      These may be fractional.
    :param dim: the dimension of the output.
    :param max_period: controls the minimum frequency of the embeddings.
    :return: an [B, T, dim] or [B, dim] Tensor of positional embeddings.
    """
    # Ensure timesteps is at least 2D: [B, T] or [B, 1] if originally [B]
    if timesteps.dim() == 1:
        if not repeat_only:
            half = dim // 2
            freqs = torch.exp(
        ...
        B, T = timesteps.shape  # Get batch and sequence length
        if not repeat_only:
            half = dim // 2
            freqs = torch.exp(
                -math.log(max_period) * torch.arange(start=0, end=half, dtype=torch.float32) / half
            ...
            # Broadcast to match shape [B, T, half]
            ...
            embedding = repeat(timesteps, 'b t -> b t d', d=dim)
        return embedding
    else:
